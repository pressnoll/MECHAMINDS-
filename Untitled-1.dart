...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    print("MyApp: Setting initial route based on auth state");
    return MaterialApp(
      title: "MECHAMINDS",
      ...
      routes: {
        ...
        '/login': (context) => LoginPage(), // Updated route for LoginPage
  ...
  Future<void> fetchStickLocation() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final latestKey = data.keys.lastOrNull;
        if (latestKey == null || data[latestKey]?['gps_location'] == null) {
          print("⚠️ No valid GPS location found in data");
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text("No GPS location in stick data")),
          );
          return;
        }

        final String gps = data[latestKey]['gps_location'];
        print("📍 Raw GPS string: $gps");

        final parts = gps.split(',');
        if (parts.length != 2) {
          print("❌ GPS data does not contain exactly 2 parts: $gps");
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text("Invalid GPS data format")),
          );
          return;
        }

        final lat = double.tryParse(parts[0].trim().split(" ")[0]);
        final lon = double.tryParse(parts[1].trim().split(" ")[0]);
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: ListView(
          children: [
            ListTile(
              title: Text("Default Map Style", style: GoogleFonts.poppins(fontSize: 16)),
              subtitle: Text(
            ...
            ListTile(
              title: Text("Default Zoom Level", style: GoogleFonts.poppins(fontSize: 16)),
              subtitle: DropdownButton<String>(
                ...
                items: _zoomLevels.keys.map((String percentage) {
                  return DropdownMenuItem<String>(
                    value: percentage,
                    child: Text(percentage, style: GoogleFonts.poppins(fontSize: 14)),
                  );
                }).toList(),
            ...
            SwitchListTile(
              title: Text("Sound Alerts", style: GoogleFonts.poppins(fontSize: 16)),
              subtitle: Text(
            ...
            Divider(),
            ListTile(
              title: Text("Emergency Contact", style: GoogleFonts.poppins(fontSize: 16)),
              subtitle: Text(
...
class ProfileScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text("Profile"),
        backgroundColor: const Color.fromARGB(255, 138, 242, 152),
      ),
      body: Padding(
        padding: EdgeInsets.all(16.0),
        child: ListView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  ElevatedButton.icon(
                    ...
                    style: ElevatedButton.styleFrom(
                      backgroundColor: const Color.fromARGB(255, 32, 179, 51),
